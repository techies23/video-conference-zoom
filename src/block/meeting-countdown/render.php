<?php
/**
 * Render template for Zoom Meeting Countdown Block.
 *
 * @var array    $attributes Block attributes.
 * @var string   $content    Block content.
 * @var WP_Block $block      Block instance.
 */

use Codemanas\VczApi\Blocks\DetailsHelper;

if ( ! defined( 'ABSPATH' ) ) {
	exit;
}

$meeting_details  = DetailsHelper::get_meeting_details( $attributes );
$meeting_tz       = $meeting_details['timezone'] ?? 'UTC';
$show_label       = $attributes['showLabel'] ?? true;
$label            = $attributes['label'] ?? __( 'Timezone:', 'video-conferencing-with-zoom-api' );
$timezone_display = $attributes['timezoneDisplay'] ?? 'user';

if ( ! in_array( $timezone_display, [ 'user', 'meeting' ], true ) ) {
	$timezone_display = 'user';
}

$wrapper_attributes = get_block_wrapper_attributes( [
	'class'                  => 'vczapi-meeting-detail-item vczapi-meeting-detail-timezone',
	'data-vczapi-meeting-tz' => $meeting_tz,
	'data-vczapi-tz-mode'    => $timezone_display,
	'sourceType'            => $attributes['sourceType'] ?? 'current',
	'selectedMeetingPostId' => $attributes['selectedMeetingPostId'] ?? 0,
	'customMeetingId'       => $attributes['customMeetingId'] ?? '',
] );

$start_time_raw  = $meeting_details['start_time'] ?? '';
$duration        = (int) ( $meeting_details['duration'] ?? 60 );
$layout_preset   = $attributes['layoutPreset'] ?? 'cards';
$unit_bg         = $attributes['unitBackgroundColor'] ?? '';
$label_size      = $attributes['labelFontSize'] ?? '0.75rem';
$started_text    = $attributes['startedText'] ?? __( 'Meeting has started', 'video-conferencing-with-zoom-api' );
$ended_text      = $attributes['endedText'] ?? __( 'Meeting has ended', 'video-conferencing-with-zoom-api' );
$show_days       = $attributes['showDays'] ?? true;
$show_seconds    = $attributes['showSeconds'] ?? true;
$all_day_started = $attributes['keepStartedAllDay'] ?? false;

if ( empty( $start_time_raw ) ) {
	return;
}

$utc_start_time = gmdate( 'Y-m-d\TH:i:s\Z', strtotime( $start_time_raw ) );

// Fetch native Gutenberg wrapper attributes first
$wrapper_attributes = get_block_wrapper_attributes( [
	'class'                       => 'vczapi-meeting-countdown vczapi-meeting-countdown--' . sanitize_html_class( $layout_preset ),
	'data-vczapi-utc-start'       => $utc_start_time,
	'data-vczapi-duration'        => $duration,
	'data-vczapi-started-text'    => esc_attr( $started_text ),
	'data-vczapi-ended-text'      => esc_attr( $ended_text ),
	'data-vczapi-show-days'       => $show_days ? '1' : '0',
	'data-vczapi-show-seconds'    => $show_seconds ? '1' : '0',
	'data-vczapi-all-day-started' => $all_day_started ? '1' : '0',
] );

// Dynamically build inline styles for theme compatibility
// Dynamically build inline styles for colors and typography
$custom_vars = '';

// 1. Color Attributes
if ( ! empty( $attributes['style']['color']['text'] ) ) {
	$text_color = sanitize_hex_color( $attributes['style']['color']['text'] );

	if ( $text_color ) {
		$custom_vars .= 'color: ' . esc_attr( $text_color ) . ';';
	}
}

if ( ! empty( $attributes['style']['color']['background'] ) ) {
	$background_color = sanitize_hex_color( $attributes['style']['color']['background'] );

	if ( $background_color ) {
		$custom_vars .= 'background-color: ' . esc_attr( $background_color ) . ';';
	}
}

// 2. Typography Attributes
$typography = $attributes['style']['typography'] ?? [];

if ( ! empty( $typography['fontFamily'] ) ) {
	$custom_vars .= 'font-family: ' . esc_attr( $typography['fontFamily'] ) . ';';
}
if ( ! empty( $typography['fontSize'] ) ) {
	$custom_vars .= 'font-size: ' . esc_attr( $typography['fontSize'] ) . ';';
}
if ( ! empty( $typography['lineHeight'] ) ) {
	$custom_vars .= 'line-height: ' . esc_attr( $typography['lineHeight'] ) . ';';
}
if ( ! empty( $typography['fontWeight'] ) ) {
	$custom_vars .= 'font-weight: ' . esc_attr( $typography['fontWeight'] ) . ';';
}
if ( ! empty( $typography['letterSpacing'] ) ) {
	$custom_vars .= 'letter-spacing: ' . esc_attr( $typography['letterSpacing'] ) . ';';
}
if ( ! empty( $typography['textTransform'] ) ) {
	$custom_vars .= 'text-transform: ' . esc_attr( $typography['textTransform'] ) . ';';
}
if ( ! empty( $typography['fontStyle'] ) ) {
	$custom_vars .= 'font-style: ' . esc_attr( $typography['fontStyle'] ) . ';';
}

// 3. Custom CSS Variables
if ( ! empty( $unit_bg ) ) {
	$custom_vars .= '--vczapi-unit-bg: ' . esc_attr( $unit_bg ) . ';';
}
if ( ! empty( $label_size ) ) {
	$custom_vars .= '--vczapi-label-size: ' . esc_attr( $label_size ) . ';';
}
if ( ! empty( $custom_vars ) ) {
	if ( str_contains( $wrapper_attributes, 'style="' ) ) {
		// Inject into existing style attribute generated by WP
		$wrapper_attributes = str_replace( 'style="', 'style="' . esc_attr( $custom_vars ), $wrapper_attributes );
	} else {
		// Create style attribute if none exists
		$wrapper_attributes .= ' style="' . esc_attr( $custom_vars ) . '"';
	}
}
?>

<div <?php echo $wrapper_attributes; ?>>
    <div class="vczapi-countdown-wrapper">
        <div class="vczapi-countdown-timer">
			<?php if ( $show_days ) : ?>
                <div class="vczapi-countdown-unit vczapi-countdown-unit--days">
                    <span class="vczapi-countdown-value">- -</span>
                    <span class="vczapi-countdown-label"><?php esc_html_e( 'DAYS', 'video-conferencing-with-zoom-api' ); ?></span>
                </div>
				<?php if ( $layout_preset === 'minimal' ) : ?>
                    <span class="vczapi-countdown-separator">:</span>
				<?php endif; ?>
			<?php endif; ?>

            <div class="vczapi-countdown-unit vczapi-countdown-unit--hours">
                <span class="vczapi-countdown-value">- -</span>
                <span class="vczapi-countdown-label"><?php esc_html_e( 'HOURS', 'video-conferencing-with-zoom-api' ); ?></span>
            </div>

			<?php if ( $layout_preset === 'minimal' ) : ?>
                <span class="vczapi-countdown-separator">:</span>
			<?php endif; ?>

            <div class="vczapi-countdown-unit vczapi-countdown-unit--minutes">
                <span class="vczapi-countdown-value">- -</span>
                <span class="vczapi-countdown-label"><?php esc_html_e( 'MINUTES', 'video-conferencing-with-zoom-api' ); ?></span>
            </div>

			<?php if ( $show_seconds ) : ?>
				<?php if ( $layout_preset === 'minimal' ) : ?>
                    <span class="vczapi-countdown-separator">:</span>
				<?php endif; ?>
                <div class="vczapi-countdown-unit vczapi-countdown-unit--seconds">
                    <span class="vczapi-countdown-value">- -</span>
                    <span class="vczapi-countdown-label"><?php esc_html_e( 'SECONDS', 'video-conferencing-with-zoom-api' ); ?></span>
                </div>
			<?php endif; ?>
        </div>
        <div class="vczapi-countdown-status-message" style="display: none;"></div>
    </div>
</div>