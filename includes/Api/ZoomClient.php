...
use Codemanas\VczApi\Backend\Debugger;
...
class ZoomClient {
	...
	protected function sendRequest( $calledFunction, $data, $request = "GET" ) {
		...
		if ( is_wp_error( $request ) ) {
			...
			if ( ! empty( $debug_log ) ) {
				if ( $responseCode == 400 ) {
					$debugger->logMessage( $responseBody, $responseCode, $request );
			...
			if ( $debugger->isValidXML( $responseBody ) ) {
				$responseBody = simplexml_load_string( $responseBody );
			} else if ( $debugger->isJson( $responseBody ) ) {
				$responseBody = json_decode( $responseBody );
			}
		}


		$response = apply_filters( 'vczapi_checkApiResponse', $responseBody, $calledFunction, $data, $request );
